 ...
 */
add_action('admin_init', 'harrington_admin_init');
function harrington_admin_init(){
    ...
    add_settings_field(
        ...
        __('Project Page', 'harrington'), // Title
        'harrington_setting_callback_function', // Callback
        'reading', // page
        'default', // section
 ...
 */
function harrington_setting_callback_function($args){
 ...
 */
add_filter('display_post_states', 'harrington_add_custom_post_states');

function harrington_add_custom_post_states($states) {
    global $post;
    ...
    if( 'page' == get_post_type($post->ID) && $post->ID == $project_page_id && $project_page_id != '0') {
        $states[] = __('AW Projects', 'harrington');
    }
